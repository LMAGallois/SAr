...
//display_array(contacts);
    ...
    cout << "nb contacts" << its_data.nb_contacts << endl;
//display_array(contacts);

    const char* cfile_creneaux = "in_out/creneaux.txt";
    ifstream file1(cfile_creneaux);
    ...
    for(int i=0; i < its_data.nbSatellites ; i++){
        creneaux.push_back(c_s);
    }
    //creneaux[0].push_back({5,0,0});
    //cout << "coucou" <<  creneaux[0].front().sat<<endl;

    schedule tempo;

    while(file1 >> tempo.sat){
        //cout << "sat" << tempo.sat << endl;
        file1 >> tempo.s ;
        //cout << "s" << tempo.s << endl;
        file1 >> tempo.e ;
        //cout << "e" << tempo.e << endl;
        //cout << tempo.sat << endl;
        if(tempo.sat!=-1){
            creneaux[tempo.sat].push_back(tempo);
                        //cout << "creneaux" << creneaux[tempo.sat].front().s << endl;
...
//display_2Dmatrix(creneaux);
    ...
    const char* cfile_userToSat = "in_out/userToSat.txt";
    ifstream file3(cfile_userToSat);

        if ( !file3 ) {
            cerr << "No such file: " << cfile_userToSat << endl;
            throw(-1);
        }
    ...
    for(int i=0; i < its_data.nbUsers ; i++){
        ...
        for(int j=0; j <its_data.nbSatellites; j++){
            ...
            for (int k =0; k < contacts.size(); k++){
                file3 >> temp;
                userToSat[i][j].push_back(temp);
            //cout << "contacts size " << i+j+k << endl;
...
//display_3Dmatrix(userToSat);
    ...
    const char* cfile_antennaToSat = "in_out/antennaToSat.txt";
    ifstream file4(cfile_antennaToSat);

    if ( !file4 ) {
        cerr << "No such file: " << cfile_antennaToSat << endl;
        throw(-1);
    }
    ...
    for(int i=0; i < its_data.nbSites*its_data.nbAntennas ; i++){
        ...
        for(int j=0; j < its_data.nbSatellites; j++){
            ...
            for (int k =0; k < contacts.size(); k++){
                file4 >> temp;
                antennaToSat[i][j].push_back(temp);
            }
        }
    }
...
int main(){
    ...
    list<schedule> plan;
    get_values ( its_data, userToSat, antennaToSat, creneaux, contacts);
//        display_3Dmatrix(userToSat);
//    display_3Dmatrix(antennaToSat);

    cout << "données pretes" << endl;
