                    ...
                    if(s1!=s2)
                    {
                        ...
                        for(int m=0; m< tabSat[s2].tabStartTimeOS.size(); m++)
                        {
                            ...
                            if(((tabSat[s1].tabMergedContactSat[j].GetStartTime() >= tabSat[s2].tabStartTimeOS[m]) and
                                (tabSat[s2].tabStartTimeOS[m] +OSContactDuration >  tabSat[s1].tabMergedContactSat[j].GetStartTime())))
                    ...
                    if(s1!=s2)
                    {
                        ...
                        for(int m=0; m< tabSat[s2].tabStartTimeOS.size(); m++)
                        {
                            ...
                            if(((tabSat[s1].tabStartTimeOS[j] >= tabSat[s2].tabStartTimeOS[m]) and
                                (tabSat[s2].tabStartTimeOS[m] +OSContactDuration >  tabSat[s1].tabStartTimeOS[j])))
...
//        }
        ...
        for (int s = 0; s < nbSat; s++)
        {
            for(int j=0; j< tabSat[s].tabStartTimeOS.size(); j++)
            {
                ...
                for(int k=0; k<nbSites; k++)
                {
                    if (tabSat[s].VisibleInLocation( ContactSat(-1,-1,-1,tabSat[s].tabStartTimeOS[j],tabSat[s].tabStartTimeOS[j]+OSContactDuration ),tabS[k]) ==0)
                    {
        ...
        cplex.setParam(IloCplex::TiLim, 100);    // time out
        cplex.setParam(IloCplex::EpGap, 0.02);     // duality gap set to 2%
        cplex.solve();
                ...
                for(int l=0; l<nbSites; l++)
                {
                    if(cplex.getValue(e[s][j][l])==1)
                    {
                        ...
                        tabS[l].ScheduleCAntenna[0].push_back(Contact(s,
                                                                      tabSat[s].tabStartTimeOS[j] , tabSat[s].tabStartTimeOS[j] +OSContactDuration));
